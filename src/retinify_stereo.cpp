...
: Node("retinify_stereo_node", options)
{
    ...
    this->declare_parameter<bool>("pointcloud_color", true);
    this->declare_parameter<std::string>("depth_mode", "accurate");
    this->declare_parameter<std::string>("calibration_file", "");
    this->declare_parameter<std::string>("frame_id", "left_camera_optical_frame");
    ...
    pointcloud_color_ = this->get_parameter("pointcloud_color").as_bool();
...
bool RetinifyStereoNode::initializePipeline(uint32_t width, uint32_t height)
{
    ...
    // Allocate / resize reusable buffers
    disparity_buffer_.resize(width * height);
    pointcloud_buffer_.resize(width * height * 3);
    ...
    RCLCPP_INFO(this->get_logger(),
        "Retinify stereo pipeline successfully initialized for resolution %ux%u with DepthMode=%s",
    ...
    const sensor_msgs::msg::Image::ConstSharedPtr msg_right)
{
    ...
    this->get_parameter("pointcloud_color", pointcloud_color_);
    frame_id_ = this->get_parameter("frame_id").as_string();
    ...
    if (publish_disparity_ || debug_image_ || publish_pointcloud_) {
        ...
        if (publish_disparity_ && pub_disparity_) {
            ...
            std::memcpy(disp_msg->image.data.data(), disparity_buffer_.data(), disp_msg->image.data.size());
    ...
    // 2. Point Cloud Retrieval & Publishing via Retinify
    if (publish_pointcloud_ && pub_pointcloud_) {
        auto pc_status = pipeline_.RetrievePointCloud(
            pointcloud_buffer_.data(), width * 3 * sizeof(float));
        ...
        float max_dist_sq = (max_dist_ > 0.0) ? static_cast<float>(max_dist_ * max_dist_) : -1.0f;
        float min_d = static_cast<float>(min_disp_);

        const uint8_t * left_img_data = left_img.ptr<uint8_t>();
        int left_stride = static_cast<int>(left_img.step[0]);

        // Point cloud extraction (multi-threaded with OpenMP if available)
#ifdef _OPENMP
        int num_threads = omp_get_max_threads();
#else
        int num_threads = 1;
#endif
        std::vector<std::vector<PointXYZRGB>> thread_points(num_threads);
...
#endif
        {
            ...
            int tid = 0;
#endif
            auto & local_pts = thread_points[tid];
            local_pts.clear();
            local_pts.reserve((crop_w * crop_h) / (step * step * num_threads));

#ifdef _OPENMP
            #pragma omp for schedule(static)
#endif
            for (int v = v0; v < v1; v += step) {
                const float * pc_row = pointcloud_buffer_.data() + (v * static_cast<int>(width) * 3);
                const float * disp_row = disparity_buffer_.data() + (v * static_cast<int>(width));
                const uint8_t * rgb_row = left_img_data + (v * left_stride);

                for (int u = u0; u < u1; u += step) {
                    float d = disp_row[u];
                    if (d < min_d) continue;

                    float X = pc_row[u * 3 + 0];
                    float Y = pc_row[u * 3 + 1];
                    float Z = pc_row[u * 3 + 2];

                    if (Z <= 0.0f || std::isnan(Z) || std::isinf(Z)) continue;

                    // Apply optional coordinate transformation
                    float xt = R_combined_[0] * X + R_combined_[1] * Y + R_combined_[2] * Z + T_combined_[0];
                    float yt = R_combined_[3] * X + R_combined_[4] * Y + R_combined_[5] * Z + T_combined_[1];
                    float zt = R_combined_[6] * X + R_combined_[7] * Y + R_combined_[8] * Z + T_combined_[2];

                    if (max_dist_sq > 0.0f) {
                        float dist_sq = xt * xt + yt * yt + zt * zt;
                        if (dist_sq > max_dist_sq) continue;
                    }

                    uint32_t rgb = 0;
                    if (pointcloud_color_) {
                        uint8_t r = rgb_row[u * 3 + 0];
                    ...
                    local_pts.push_back({xt, yt, zt, rgb});
                }
            }
        }

        size_t total_points = 0;
        for (const auto & pts : thread_points) {
        ...
        cloud_msg->header.frame_id = (tf_static_cached_ && !parent_frame_.empty()) ? parent_frame_ : frame_id_;
        cloud_msg->height = 1;
        ...
        cloud_msg->point_step = sizeof(PointXYZRGB);
        ...
        for (const auto & pts : thread_points) {
            if (!pts.empty()) {
                std::memcpy(cloud_msg->data.data() + offset, pts.data(), pts.size() * sizeof(PointXYZRGB));
                offset += pts.size() * sizeof(PointXYZRGB);
            }
        }
