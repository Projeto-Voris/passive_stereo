...
__global__ void triangulate_kernel(
    ...
    PointT* __restrict__ d_out_points,
    ...
    size_t max_points)
{
    ...
    if (thread_in_bounds) {
        ...
        if (d > params.min_disp) {
            ...
            bool dist_ok = (Z > 0.0f || params.invert_z);
            ...
            if (dist_ok && params.max_dist_sq > 0.0f) {
            ...
            float conf = 1.0f;
            if (dist_ok) {
                // Confidence-based noise gate (local disparity variance)
                if (params.confidence_radius > 0) {
                    ...
                    if (count > 1) {
                        ...
                        // Isolated disparity pixel without valid neighbors (speckle noise)
                        dist_ok = false;
                    }
                }
            }
            ...
            if (dist_ok) {
                ...
                if constexpr (std::is_same_v<PointT, PointXYZRGBConfidence>) {
                    pt.confidence = conf;
                }
            }
        }
...
bool CudaTriangulator::ensure_device_buffers(size_t disp_bytes, size_t img_bytes, size_t points_bytes)
{
...
bool CudaTriangulator::ensure_pinned_output(size_t points_bytes)
{
    if (h_pinned_points_cap_ < points_bytes) {
        if (h_pinned_points_) cudaFreeHost(h_pinned_points_);
        size_t alloc_sz = points_bytes + (points_bytes / 4);
        if (cudaHostAlloc(&h_pinned_points_, alloc_sz, cudaHostAllocDefault) != cudaSuccess) {
            h_pinned_points_ = nullptr;
            h_pinned_points_cap_ = 0;
            return false;
        }
        h_pinned_points_cap_ = alloc_sz;
    }
    return true;
}

template <typename PointT>
size_t CudaTriangulator::triangulate_impl(
    ...
    PointT**       h_points_out)
{
    ...
    size_t points_bytes = max_points * sizeof(PointT);
    if (!ensure_device_buffers(disp_bytes, img_bytes, points_bytes) ||
        !ensure_pinned_input(disp_bytes, img_bytes) ||
        !ensure_pinned_output(points_bytes)) {
        if (h_points_out) *h_points_out = nullptr;
        return 0;
    }
    ...
    triangulate_kernel<PointT><<<grid, block, 0, stream>>>(
        d_disparity_,
        d_image_,
        params,
        reinterpret_cast<PointT*>(d_points_),
        d_count_,
        max_points);

    // 5. Sync and download valid points
    if (!h_count_mapped_) {
        ...
        if (valid_points > 0) {
            cudaMemcpyAsync(h_pinned_points_, d_points_,
                            valid_points * sizeof(PointT), cudaMemcpyDeviceToHost, stream);
            cudaStreamSynchronize(stream);
        }
        *h_points_out = reinterpret_cast<PointT*>(h_pinned_points_);
        return valid_points;
    }
...
{
    return triangulate_impl<PointXYZRGB>(
        h_disparity, disp_bytes, h_image, img_bytes, params, max_points, h_points_out);
}
...
{
    return triangulate_impl<PointXYZRGBConfidence>(
        h_disparity, disp_bytes, h_image, img_bytes, params, max_points, h_points_out);
}
